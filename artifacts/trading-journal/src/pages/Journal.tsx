...
import { buildBalanceMap } from "@/lib/runningBalance";
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="bg-secondary/50 text-muted-foreground uppercase text-xs">
              <tr>
                ...
                <th className="px-4 py-3 font-medium text-right">Bal Before</th>
                <th className="px-4 py-3 font-medium text-right">Bal After</th>
              ...
              {filteredAndSorted.length === 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium max-w-[130px] truncate">{t.asset === "Custom" ? t.customAsset : t.asset}</td>
                    <td className="px-4 py-3 uppercase text-xs">{t.direction}</td>
                    <td className="px-4 py-3 text-right text-muted-foreground">{t.lotSize ?? "—"}</td>
                    <td className="px-4 py-3 text-right text-muted-foreground">{t.entryPrice ? `$${t.entryPrice}` : "—"}</td>
                    <td className="px-4 py-3 text-right text-muted-foreground">{t.exitPrice ? `$${t.exitPrice}` : "—"}</td>
                    ...
                    <td className={`px-4 py-3 text-right font-medium ${bal ? (bal.balanceAfter >= bal.balanceBefore ? "text-success" : "text-destructive") : ""}`}>
                      {bal ? fmt(bal.balanceAfter) : "—"}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {t.tags.slice(0, 2).map((tag, i) => (
                          <span key={i} className="bg-secondary text-secondary-foreground text-[10px] px-1.5 py-0.5 rounded truncate max-w-[80px]">{tag}</span>
                        ))}
